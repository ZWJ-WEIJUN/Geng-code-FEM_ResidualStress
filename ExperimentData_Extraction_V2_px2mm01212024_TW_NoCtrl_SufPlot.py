...
with open('TW70_LP_CTRL_Run20240128.npy', 'rb') as f:
    ...
    Coord_TW_3D_40layers_LPCtrl = np.load(f, allow_pickle=True) # Coord_TW_3D_40layers_LPCtrl is the thin wall coordinate in 3D, shape is (40, 8, 3)  
    ...
    # Exclude the first element of Frame_index_LPCtrl because the first element is ZERO, which is not a valid index for the Frame_history_LPCtrl.
    ...
    Time_Stamp_LPCtrl = np.load(f, allow_pickle=True) # Time_Stamp_LPCtrl is the time stamp for each frame collected by the IR camera during the experiment
...
# Extract the first sublist from Coord_TW_3D_40layers_LPCtrl
first_sublist_LPCtrl = Coord_TW_3D_40layers_LPCtrl[0]
# Extract the second element from each sublist in first_sublist
second_elements_LPCtrl = [sublist[1] for sublist in first_sublist_LPCtrl]

x_labels_LPCtrl = np.array(second_elements_LPCtrl, dtype=int)



# Convert the MT_OriginSquareSearch_alllayers to a numpy array
MT_OriginSquareSearch_alllayers = np.array(MT_OriginSquareSearch_alllayers, dtype=float)

# # ***********************  Heatmap plot - START - new figure for saving as .svg format
...
for i in range(MT_OriginSquareSearch_alllayers.shape[0]):
    for j in range(MT_OriginSquareSearch_alllayers.shape[1]):
        plt.text(j, i, format(MT_OriginSquareSearch_alllayers[i, j], '.1f'),
                 horizontalalignment="center",
                 verticalalignment="center",
                 color="white" if MT_OriginSquareSearch_alllayers[i, j] > 900 else "black", )

# Set up the labels for the x and y axes
plt.xlabel('Coordinate along machine Y axis (mm)',  fontname='Arial Black')
plt.ylabel('Layers #', fontname='Arial Black')
...
x = np.arange(MT_RecSearch_alllayers.shape[1])  # column index
y = np.arange(1, MT_RecSearch_alllayers.shape[0] + 1)  # row (Thin Wall Layer #) index starting from 1
X, Y = np.meshgrid(x, y)
Z = MT_RecSearch_alllayers
print(MT_RecSearch_alllayers)
...
# ***********************  3D Surface plot add LP_Ctrl
MT_OriginSquareSearch_alllayers_LPCtrl = np.array(MaxT_OriginRectSearch_alllayers_LPCtrl, dtype=float)

# Create the x, y, and z coordinates for the surface plot
x2 = np.arange(MT_OriginSquareSearch_alllayers_LPCtrl.shape[1])  # column index
y2 = np.arange(1, MT_OriginSquareSearch_alllayers_LPCtrl.shape[0] + 1)  # row (Thin Wall Layer #) index starting from 1
X2, Y2 = np.meshgrid(x2, y2)
Z2 = MT_OriginSquareSearch_alllayers_LPCtrl
print(MT_OriginSquareSearch_alllayers_LPCtrl)

# Use interpolation to make the surface plot smoother
X_smooth2 = np.linspace(X2.min(), X2.max(), 200)
Y_smooth2 = np.linspace(Y2.min(), Y2.max(), 1000)
X_smooth2, Y_smooth2 = np.meshgrid(X_smooth2, Y_smooth2)
Z_smooth2 = griddata((X2.flatten(), Y2.flatten()), Z2.flatten(), (X_smooth2, Y_smooth2), method='cubic')

# Create the surface plot with the smoothed data
surf2 = ax.plot_surface(X_smooth2, Y_smooth2, Z_smooth2, cmap='Reds', edgecolor='none')  # Remove the mesh

# Set up the ticks for the x and y axes
ax.set_xticks(ticks=np.arange(len(x_labels_LPCtrl)), labels=x_labels_LPCtrl)
...
Z_diff = Z_smooth - Z_smooth2  # Calculate the difference between the two surfaces

# Create the surface plot for the difference
surf_diff = ax.plot_surface(X_smooth2, Y_smooth2, Z_diff, cmap='Greens', edgecolor='none')  # Use 'seismic' colormap for the difference

# Set up the labels for the x, y, and z axes
ax.set_xlabel('Coordinate along machine Y axis (mm)', fontname='Arial Black')
ax.set_ylabel('Layer #', fontname='Arial Black')
ax.set_zlabel('Temperature Difference (°C)', fontname='Arial Black')


plt.title('Temperature Distribution - (LP Ctrl vs. No Ctrl)', fontname='Arial Black')  # Add a title to the plot
# Add a legend for the surface plot
ax.legend([surf, surf2, surf_diff], ['No Ctrl', 'LP Ctrl', 'Difference'], loc='upper right')
