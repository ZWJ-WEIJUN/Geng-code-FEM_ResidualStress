# -*- coding: utf-8 -*-
"""
Created on Sun Jun  2 11:23:02 2024
...
if __name__ == '__main__':
    ...
    file_path = 'C:/Users/muqin/OneDrive/Desktop/Reasearch/Width_Data_Analysis/Width_Data_Analysis/STW70_LP_and_FR_Ctrl_1100.csv'
    ...
    ax3.scatter(LP_df[0], LP_df[1],color=color_lp, s=5, marker='.',label='STW 70 LP Control')
    ax3.scatter(FR_df[0]-X_shift, FR_df[1],color=color_fr,s=5 ,  marker='.',label='STW 70 FR Control' )
    
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.legend(scatterpoints=1,loc='lower center',ncol=3,fontsize=8,markerscale = 5)

    
    print('LP_CTRL_Mean:',LP_mean)
    print('LP_CTRL_SD:',LP_SD)
    print('FR_CTRL_Mean:',FR_mean)
    print('FR_CTRL_SD:',FR_SD)
    ...
    plt.ylim(2.5,3.8)
    plt.title('LP Control Step Thin Wall with 1100$^\circ$C Objective')
    ...
    plt.ylim(2.5,3.8)
    plt.title('FR Control Step Thin Wall with 1100$^\circ$C Objective')
    plt.xlabel('Height(mm)')
    plt.ylabel('Width(mm)')
    # plt.legend(scatterpoints=1,loc='lower center',ncol=2,fontsize=8,markerscale = 1.0)
    handles, labels = plt.gca().get_legend_handles_labels()
