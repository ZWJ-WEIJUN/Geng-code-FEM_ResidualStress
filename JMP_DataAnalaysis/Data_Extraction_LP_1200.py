...
print(f'The medium temperature for each layer of No Ctrl STW is: {Medium_temperature_RecSearch31x5}')
# Calculate the difference between consecutive elements in Medium_temperature_RecSearch31x5
temperature_diff = np.diff(Medium_temperature_RecSearch31x5)
print(f"The difference between consecutive elements in Medium_temperature_RecSearch31x5 is: {temperature_diff}")


num_elements = len(temperature_diff)
print(f"The number of elements in temperature_diff is: {num_elements}")

# Calculate the percentage of elements in temperature_diff that are less than 50
print(f'The number of elements in temperature_diff that are less than 30 is: {np.count_nonzero(temperature_diff < 30)}')
percentage_less_than_30 = (np.count_nonzero(temperature_diff < 30) / (num_elements)) * 100
print(f"The percentage of elements in temperature_diff that are less than 30 is: {percentage_less_than_30}%")
...
ax1.axvspan(0, 30, facecolor='crimson', alpha=0.1)
ax1.axvspan(30, 60, facecolor='crimson', alpha=0.3)
ax1.axvspan(60, 90, facecolor='crimson', alpha=0.5)
...
print(f"Laser power array: {lp_array[1:]*1000}")
...
lns3 = ax2.plot(layer, Medium_temperature_OriginalRectSearch, linewidth=3, marker='.', markersize=10, linestyle='-', color=color_T,label='Median Temperature - LP Control')
# Plot Medium_temperature_RecSearch
print(f'The temperature distribution for the LP Ctrl is:{Medium_temperature_OriginalRectSearch}')
...
plt.legend(lns, labs, loc='lower center',fontsize=17,bbox_to_anchor=(0.4, 0.0))

# Set up the title for the plot
# plt.title('Medium temperature acqureied by different search methods')
plt.title("LP Control Step Thin Wall with 1200$^\circ$C Objective", fontname="Times New Roman",
