...
lns1 = ax1.plot(layer, lp_array[1:]*1000, linewidth=3, marker='.', markersize=10, color=color_lp, label='Laser power (W)')
ax1.set_ylabel('Laser Power (W)', color=color_lp,size=17)
ax1.tick_params(axis='y', labelcolor='black',labelsize=17)
ax1.set_xlabel("Layer #", fontweight='bold',size=17)
ax1.set_ylabel("Machine Parameters", color='black', fontweight='bold',size=17)
...
lns3 = ax2.plot(layer, Medium_temperature_OriginalRectSearch, linewidth=3, marker='.', markersize=10, linestyle='-', color=color_T,label='Median Temperature - Combined II Control')
# Plot Medium_temperature_RecSearch
print(f"Medium temperature for hybrid control: {Medium_temperature_OriginalRectSearch}")
print(f"The feed rate values are: {fr_array[1:]*60}")
print(f"The laser power values are: {lp_array[1:]*1000}")
...
lns4 = ax2.plot(layer, Medium_temperature_RecSearch31x5, linewidth=3, marker='.', markersize=10, linestyle='-', color='black', label ='Median Temperature - No Control')
lns5 = ax1.plot(layer, fr_array[1:]*60, linewidth=3, marker='.', markersize=10, color=color_fr, label='Feed rate(mm/min)') 
ax2.set_ylabel('Median_temperature ($^\circ$C)', color=color_T, fontweight='bold',size=17)
ax2.tick_params(axis='y', labelcolor=color_T,labelsize=17)
...
plt.legend(lns, labs, loc='lower right', fontsize=17)

# Set up the title for the plot
# plt.title('Medium temperature acqureied by different search methods')
plt.title("Combined II Control Step Thin Wall with 1200$^\circ$C Objective", fontname="Times New Roman",
...
for index, frame_index_i in enumerate(Frame_index):
    ...
    ax.legend(handles=legend_elements, fontsize=17, loc='lower right')

    # Display the first frame as an image
    plt.imshow(frame, cmap='coolwarm', interpolation='nearest')

    # Add a colorbar to the right of the plot
    # plt.colorbar(label='Value' ,labelsize=17)
    cbar = plt.colorbar()
    cbar.ax.tick_params(labelsize=17)
    # Set up the title for the plot
    plt.title(f'Layer #{index+1}',fontsize=17)
