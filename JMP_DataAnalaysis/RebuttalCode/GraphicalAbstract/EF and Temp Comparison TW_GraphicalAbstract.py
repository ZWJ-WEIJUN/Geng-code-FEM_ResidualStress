...
NC_Temp = np.load('TW25_No_Ctrl_run20240510.npy', allow_pickle=True)
NC_Temp = np.median(NC_Temp, axis=1)
...
def stat_cal(temp,title):
    # short_data = temp[np.r_[30:90]]
    short_data = temp[np.r_[30:45,46:90]]
    short_temp_mean = np.mean(short_data)
    short_temp_sd = np.std(short_data)
    ...
    print(title+' short std:'+str(short_temp_sd))
    # return mid_temp_mean,mid_temp_sd,short_temp_mean,short_temp_sd, mid_lower, mid_upper, short_lower, short_upper
    return short_temp_mean,short_temp_sd, short_CI
...
NC_short_temp_mean,NC_short_temp_sd, NC_short_CI = stat_cal(NC_Temp,'NC')
LP_short_temp_mean,LP_short_temp_sd, LP_short_CI = stat_cal(LP_Temp,'LP')
FR_short_temp_mean,FR_short_temp_sd, FR_short_CI = stat_cal(FR_Temp,'FR')
HC_short_temp_mean,HC_short_temp_sd, HC_short_CI = stat_cal(HC_Temp,'HC')
...
Temp_mean_data = [[NC_short_temp_mean],[LP_short_temp_mean],
                  [FR_short_temp_mean],[HC_short_temp_mean]]
Temp_sd_data = [[NC_short_temp_sd],[LP_short_temp_sd],
                  [FR_short_temp_sd],[HC_short_temp_sd]]
Temp_CI_data = [[NC_short_CI],[LP_short_CI],
                  [FR_short_CI],[HC_short_CI]]

print(f'Mean for No Ctrl: {NC_short_temp_mean}')
print(f'Mean for LP Ctrl: {LP_short_temp_mean}')
print(f'Mean for FR Ctrl: {FR_short_temp_mean}')
print(f'Mean for Hybrid Ctrl: {HC_short_temp_mean}')

print(f'Confidence Interval for No Ctrl: {NC_short_CI}')
print(f'Confidence Interval for LP Ctrl: {LP_short_CI}')
print(f'Confidence Interval for FR Ctrl: {FR_short_CI}')
print(f'Confidence Interval for Hybrid Ctrl: {HC_short_CI}')
