...
with open('TW25_Combined_II_1200_Obj_v220240515.npy', 'rb') as f:
    ...
    Loop_Time = np.load(f, allow_pickle=True) 
    Loop_Time = np.diff(Loop_Time)
...
print(f"The shape of the Loop_Time is: {Loop_Time.shape}")
print(f"The run time of each loop is: {Loop_Time}")
...
plt.plot(layer, EF[1:], linewidth=3, marker='.', markersize=10, linestyle='-', color='green',label='Energy Fluence')
# Set up the labels for the x and y axes
plt.xlabel('Layer Number #')
plt.ylabel('Energy Fluence ($J/mm^2$)')
plt.title("Hybrid Control Thin Wall with 1200$^\circ$C Objective", fontname="Times New Roman",
...
plt.scatter(indices, OPCUA_Read_Time, color='orange', label='OPCUA read time duration', s=15)
plt.scatter(Frame_index, OPCUA_Write_Time, color='black', label='OPCUA write time duration', s=15)
plt.scatter(indices[:-1], Loop_Time, color='blue', label='Loop time duration', s=15)
plt.xlabel('While Loop Index',fontweight='bold',size=22)
plt.ylabel('Time Spent (s)',fontweight='bold',size=22)
plt.title('Python Run Time for Each loop with Every Frame Saved', fontweight='bold', size=22) # Each while loop including: OPC UA communication read R varaibles from the machine, OPC UA communication write R varibles to the machine, DPSS angle adjustment, and frame reading and saving time
plt.ylim(-0.2,1.4)
...
Medium_temperature_RecSearch31x5 = np.load('TW25_No_Ctrl_run20240510.npy', allow_pickle=True)
Medium_temperature_RecSearch31x5 = np.median(Medium_temperature_RecSearch31x5, axis=1)
# *************************************************************************
...
lns1 = ax1.plot(layer, lp_array[1:]*1000, linewidth=3, marker='.', markersize=10, color=color_lp, label='LP')
ax1.set_ylabel('Laser Power (W)', color=color_lp)
ax1.tick_params(axis='y', labelcolor='black',labelsize=30)
ax1.set_xlabel("Layer #", fontweight='bold',size=30)
ax1.set_ylabel("LP (W) and FR (mm/min)", color='black', fontweight='bold',size=30)
ax1.tick_params(axis='both', labelcolor='black',labelsize=30, width=2)
...
lns2 = ax2.plot(layer, np.full((Frame_index.shape[0], ), 1200),
                linestyle='dashed', linewidth=3, color=color_objT, label='$T_{obj}$')
lns3 = ax2.plot(layer, Medium_temperature_OriginalRectSearch, linewidth=3, marker='s', markersize=6, linestyle='-', color=color_T,label='$T_{i}$ - Hybrid Ctrl')
...
lns4 = ax2.plot(layer, Medium_temperature_RecSearch31x5, linewidth=3, marker='s', markersize=6, linestyle='-', color='black', label ='$T_{i}$ - No Ctrl')
lns5 = ax1.plot(layer, fr_array[1:]*60, linewidth=3, marker='.', markersize=10, color=color_fr, label='FR') 
ax2.set_ylabel('$T_{i}$ and $T_{obj}$ ($^\circ$C)', color='black', fontweight='bold',size=30)
ax2.tick_params(axis='both', labelcolor='black',labelsize=30, width=2)

# Set up the labels for the x and y axes
plt.xlabel('Layer Number #')
# plt.ylabel('Median Temperature (°C)')

lns = lns1+lns5+lns2+lns3+lns4
...
for index, frame_index_i in enumerate(Frame_index):
    ...
    ax.tick_params(axis='both', labelcolor='black',labelsize=22)
    ...
    for j in imge_coord_perLayer:
        ...
        ax.add_patch(square)
        rect = Rectangle((center[0]-15,center[1]-3),31,7, lw=1.0, ec='white', facecolor='none')
        ax.add_patch(rect)
    ...
    # Add a legend
    legend_elements = [
        Patch(facecolor='yellow', edgecolor='none', label='Image Coordinates from Tranfomration'),
        Patch(facecolor='green', edgecolor='none', label='Coordinates after Local Search'),
    ]
    ax.legend(handles=legend_elements, fontsize=22, loc='lower right')

    # Display the first frame as an image
    plt.imshow(frame, cmap='coolwarm', interpolation='nearest')

    # Add a colorbar to the right of the plot
    # plt.colorbar(label='Value' ,labelsize=22)
    cbar = plt.colorbar()
    cbar.ax.tick_params(labelsize=22)
    # Set up the title for the plot
    plt.title(f'Layer #{index+1}',fontsize=22)
