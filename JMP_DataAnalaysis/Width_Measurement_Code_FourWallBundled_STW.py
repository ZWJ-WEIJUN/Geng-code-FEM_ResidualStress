...
def clean_data_v1(file_path, expected_number_of_fields):
    ...
    HC =  lines[Index[0]:Index[1]-1]+lines[Index[1]:Index[2]-1]+lines[Index[2]:Index[3]-1]+lines[Index[3]:Index[4]-1]+lines[Index[4]:Index[5]-1]
    FR =  lines[Index[5]:Index[6]-1]+lines[Index[6]:Index[7]-1]+lines[Index[7]:Index[8]-1]+lines[Index[8]:Index[9]-1]+lines[Index[9]:Index[10]-1]
    LP =  lines[Index[10]:Index[11]-1]+lines[Index[11]:Index[12]-1]+lines[Index[12]:Index[13]-1]+lines[Index[13]:Index[14]-1]+lines[Index[14]:Index[15]-1]
    NC =  lines[Index[15]:Index[16]-1]+lines[Index[16]:Index[17]-1]+lines[Index[17]:Index[18]-1]+lines[Index[18]:Index[19]-1]+lines[Index[19]:Index[20]-1]
...
if __name__ == '__main__':
    ...
    file_path = '/Users/zhangweijun/Documents/GitHub/Geng-code-FEM_ResidualStress/JMP_DataAnalaysis/STW_4xThicknessProfile(H,FR,LP,No).csv'

    Combined_II_1200, STW70_FR, STW70_LP, STW70_No_Ctrl = clean_data_v1(file_path, expected_number_of_fields)
...
#**************************Scatter Plot***************************************************     
    fig, ax = plt.subplots(figsize=(10, 8))  
    No_Ctrl = ax.scatter(Height_info,No_Ctrl_width, color='black',label='No Ctrl')
    Lp = ax.scatter(Height_info,LP_width, color=color_lp, label='LP Ctrl')
    Fr = ax.scatter(Height_info,FR_width, color=color_fr, label='FR Ctrl')
    # Combined_I_1200 = ax2.scatter(Height_info,Combined_I_1200_width, color='tab:blue', label='STW 70 Combined I Control')
    Combined_II_1200 = ax.scatter(Height_info,Combined_II_1200_width, color='tab:blue', label='Hybrid Ctrl')
    
    plt.ylim(2.4,4.0)
    # plt.title('Control Method Comparison',fontname="Times New Roman", size=17, fontweight="bold")
    ax.tick_params(axis='both', labelcolor='black',labelsize=17)
...
#*************************Line Plot*************************************************** 
    fig, ax = plt.subplots(figsize=(10, 8))
    No_Ctrl = ax.plot(Height_info,No_Ctrl_width, color='black',label='No Ctrl',linewidth=3, marker='.', markersize=17, linestyle='-')
    Lp = ax.plot(Height_info,LP_width, color=color_lp, label='LP Ctrl',linewidth=3, marker='.', markersize=17, linestyle='-')
    Fr = ax.plot(Height_info,FR_width, color=color_fr, label='FR Ctrl',linewidth=3, marker='.', markersize=17, linestyle='-')
    # Combined_I_1200 = ax2.scatter(Height_info,Combined_I_1200_width, color='tab:blue', label='STW 70 Combined I Control')
    Combined_II_1200 = ax.plot(Height_info,Combined_II_1200_width, color='tab:blue', label='Hybrid Ctrl',linewidth=3, marker='.', markersize=17, linestyle='-')

    plt.ylim(2.4,4.0)
    # plt.title('1100$^\circ$C Objective Control Method Comparison',fontname="Times New Roman", size=17, fontweight="bold")
    ax.tick_params(axis='both', labelcolor='black',labelsize=17)
    plt.xlabel('Height(mm)', fontweight="bold",size=17)
    plt.ylabel('Width(mm)', fontweight="bold",size=17)
    plt.legend(scatterpoints=1,loc='lower center',ncol=1,fontsize=17,markerscale = 1.0)
 ...
 #*************************Line in section Plot STW 70 Comparison***************************************************    
    ...
    for i in range(0, len(Height_target)):
        ...
        No_Ctrl_data.append(No_Ctrl_width[i*y_range:(i+1)*y_range])
        LP_Ctrl_data.append(LP_width[i*y_range:(i+1)*y_range])
        FR_Ctrl_data.append(FR_width[i*y_range:(i+1)*y_range])
        Hybrid_Ctrl_data.append(Combined_II_1200_width[i*y_range:(i+1)*y_range])

    print("No_Ctrl_data:", No_Ctrl_data)
    print("LP_Ctrl_data:", LP_Ctrl_data)
    print("FR_Ctrl_data:", FR_Ctrl_data)
    print("Hybrid_Ctrl_data:", Hybrid_Ctrl_data)
    ...
    # plt.legend([No_Ctrl,Lp,Fr,Combined_II_1200],['No Ctrl','LP Ctrl','FR Ctrl','Hybrid Ctrl'],scatterpoints=1,loc='lower center',ncol=1,fontsize=17,markerscale = 1.0)
    plt.xlim(0, 65)
    plt.ylim(2.7, 4.2)
    ...
    ax.scatter(Combined_II_1200_df[0]-(Combined_II_1200_mid_x-LP_mid_x), Combined_II_1200_df[1],color='tab:blue',s=5 ,  marker='.',label='Hybrid Ctrl')
    ...
    plt.ylabel('Height-Z coordiante (mm)', fontweight="bold",size=17)

    # # # Calculate the angle of tilt using linear regression
    # # slope, intercept, r_value, p_value, std_err = linregress(LP_df[0], LP_df[1])
    # angle_degrees = 5.0  # convert angle to degrees if needed
    # angle_radians = np.radians(angle_degrees)  # convert angle to radians if needed
    # print(f"Angle in radians: {angle_radians}")
    # print(f"Angle in degrees: {angle_degrees}")

    # # Rotate the data points to correct the tilt
    # def rotate_points(x, y, angle):
    #     x_rot = x * np.cos(angle) + y * np.sin(angle)
    #     y_rot = -x * np.sin(angle) + y * np.cos(angle)
    #     return x_rot, y_rot

    # No_Ctrl_x_rot, No_Ctrl_y_rot = rotate_points(No_Ctrl_df[0]-(No_Ctrl_mid_x-LP_mid_x), No_Ctrl_df[1], -angle_radians)
    # LP_x_rot, LP_y_rot = rotate_points(LP_df[0], LP_df[1], -angle_radians)
    # FR_x_rot, FR_y_rot = rotate_points(FR_df[0]-(FR_mid_x-LP_mid_x), FR_df[1], -angle_radians)
    # Combined_II_1200_x_rot, Combined_II_1200_y_rot = rotate_points(Combined_II_1200_df[0]-(Combined_II_1200_mid_x-LP_mid_x), Combined_II_1200_df[1], -angle_radians)

    # ax.scatter(No_Ctrl_x_rot, No_Ctrl_y_rot, color='black', s=5, marker='.', label='NO Ctrl')  
    # ax.scatter(LP_x_rot, LP_y_rot, color=color_lp, s=5, marker='.', label='LP Ctrl')
    # ax.scatter(FR_x_rot, FR_y_rot, color=color_fr, s=5, marker='.', label='FR Ctrl')
    # ax.scatter(Combined_II_1200_x_rot, Combined_II_1200_y_rot, color='tab:blue', s=5, marker='.', label='Hybrid Ctrl')
