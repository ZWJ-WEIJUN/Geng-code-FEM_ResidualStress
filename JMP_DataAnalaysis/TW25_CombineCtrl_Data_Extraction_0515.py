# -*- coding: utf-8 -*-
"""
Created on Fri May 10 12:41:19 2024
...
def get_image_coord_ls(Layer_num: int,Frame_index,Frame_history):
    ...
    Layer_Height = 0.7
    Length = 25.                # length of the thin wall in the y direction
...
with open('TW25_Combined_II_1200_Obj_v220240515.npy', 'rb') as f:
    ...
    lp_array = 1000*lp_array
    fr_array = 60*fr_array
...
with open('TW25_No_Ctrl_run20240510.npy', 'rb') as f:
    MaxT_OriginRectSearch_alllayers_0510 = np.load(f, allow_pickle=True)  # MaxT_OriginRectSearch_alllayers is a np.ndarray that contains the temperature data collected by the IR camera during the experiment, shape is (40, 8)
Median_temperature_OriginalRectSearch_0510 = np.max(MaxT_OriginRectSearch_alllayers_0510, axis=1)   # Median temperature for each layer based on the rectangular (31column x 7row) search method  
...
plt.scatter(indices[:-1], Loop_Time, color='blue', label='Loop time duration', s=7)
...
# Median_temperature_RecSearch31x5 = np.load('SPTW_NOCtrl_01312024_Median_temperature_RecSearch_5(row)x31(column).npy', allow_pickle=True)
# # Check the shape of your data
# print("Shape of Median_temperature_RecSearch31x5:", Median_temperature_RecSearch31x5.shape)
...
lns1 = ax1.plot(layer, lp_array[1:], linewidth=3, marker='.', markersize=10, color=color_lp, label='Laser power(W)')
ax1.tick_params(axis='y', labelcolor=color_fr, )
ax1.set_xlabel("Layer #", fontweight='bold')
ax1.set_ylabel("Machine Parameter", color=color_fr, fontweight='bold')
...
lns3 = ax2.plot(layer, Median_temperature_OriginalRectSearch, linewidth=3, marker='.', markersize=10, linestyle='-', color=color_T,label='Median Temperature - Comb_TW25_05152024')
...
# lns4 = ax2.plot(layer, Median_temperature_RecSearch31x5[:,4], linewidth=3, marker='.', markersize=10, linestyle='-', color='black', label ='Median Temperature - No Ctrl_STW70')
lns5 = ax1.plot(layer, fr_array[1:], linewidth=3, marker='.', markersize=10, color=color_fr, label='Feed rate(mm/min)') 
lns6 = ax2.plot(layer, Median_temperature_OriginalRectSearch_0510, linewidth=3, marker='.', markersize=10, linestyle='-', color='black',label='Median Temperature - No CTRL_TW25_05102024')
ax2.set_ylabel('Median_temperature ($^\circ$C)', color=color_T, fontweight='bold')
ax2.tick_params(axis='y', labelcolor=color_T)

print(f"Size of Median_temperature_Hybrid Ctrl for TW25: {Median_temperature_OriginalRectSearch.size}")
print(f"Median_temperature_Hybrid Ctrl for TW25: {Median_temperature_OriginalRectSearch}")

print(f"Size of lp_array[1:]: {lp_array[1:].size}")
print(f"lp_array[1:]: {lp_array[1:]}")

print(f"Size of fr_array[1:]: {fr_array[1:].size}")
print(f"fr_array[1:]: {fr_array[1:]}")
...
plt.title("TW25 Combined Energy Contrl Test 2:05152024", fontname="Arial Black",
