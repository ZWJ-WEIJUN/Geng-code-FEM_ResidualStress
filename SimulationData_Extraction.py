...
with open('thinwall_test_no_control_lp1.7_09162023.npy', 'rb') as f:
    nodal_T2_NoCtrl = np.load(f)   # nodal_T2 * nodal_volume * heat_capacity
    nodal_A2_NoCtrl = np.load(f)  # activate
    LSC = np.load(f)
    energy_lose_NoCtrl = np.load(f)
    laser_energy_NoCtrl = np.load(f)
    median_temperature_NoCtrl = np.load(f)
    lp_layer_NoCtrl = np.load(f)
    fr_layer_NoCtrl = np.load(f)
    dp_error_NoCtrl = np.load(f)
    dp_dedl_NoCtrl = np.load(f)
...
lns3 = ax2.plot(layer, median_temperature, color=color,
                linewidth=3, marker=".", markersize=10, label='Median temperature- Offline LP Ctrl')
lns4 = ax2.plot(layer, median_temperature_NoCtrl, linewidth=3, marker='.', color='black', label='Median temperature - Offline No Ctrl' )
