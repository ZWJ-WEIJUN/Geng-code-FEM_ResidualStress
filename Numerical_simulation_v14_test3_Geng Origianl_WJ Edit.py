...
# calculate the heat capacity value by linear interpolation    
    for i in range(0,len(cp_T)-1):
        if(T<cp_T[i+1]):
            c = cp_v[i]+(cp_v[i+1]-cp_v[i])/(cp_T[i+1]-cp_T[i])*(T-cp_T[i])
            return c
...
"_____________________________________________________________________________"
# define conductivity
def k(T, k_T, k_v, k_range):
    # claim conductivity temperature array   
#    k_T = [20.,   100.,  200.,  300.,  400.,  500.,  600.,  700., 800., 900.,
#           1000., 1100., 1200., 1300., 1385., 1450., 1500., 1600.]

# claim the values of conductivity respective to the temperatures                  ? How to know all this values ?
...
# find the conductivity value of the current temperture     
    if(T>=1600.):
        return 30.5
#    elif(T<=20.):                                                                 ? Why comment out this ?
#        return 13.4                              
...
k_v = [13.4, 15.5, 17.6, 19.4, 21.8, 23.4, 24.5, 25.1, 27.2,              
          27.9, 29.1, 29.3, 30.9, 31.1, 31.0, 28.5, 29.5, 30.5]
print( k(21, k_T, k_v, range(17)) )                       
----------------------------------------------------------------------------'''


# define conductivity in binary search
def kb(T, k_T, k_v):
# claim conductivity temperature array   
#    k_T = [20.,   100.,  200.,  300.,  400.,  500.,  600.,  700., 800., 900.,
#           1000., 1100., 1200., 1300., 1385., 1450., 1500., 1600.]
...
# search for the T range using binary search         
    while left<right:
        mid = int((left+right)/2)
        if k_T[mid]<T:
            if mid==left:
                return k_v[left]+(k_v[right]-k_v[left])/(k_T[right]-k_T[left])*(T-k_T[left])
            else:
                left = mid                
        else:
            right = mid

# assuming T is always greater than 25C        
    return 30.5
...
#        return 0.0121
    ...
    if T<3000.:
        return 0.0121+(0.1921-0.0121)/3000.*T
    else:
        return 0.1921
...
# substrate bottom mesh size can be larger top mesh size since bottom temperature is not as important as top temperature     
    if(SubH1%SHM1>0.001) or (SubH2%SHM2>0.001) or (SubW%WM>0.001)  or (SubL%LM>0.001) or \
      (CladH%CHM>0.001)  or (CladW%WM>0.001)   or (CladL%LM>0.001) or                    \
      (Width%WM>0.001)   or (Width%LM>0.001)   or                                        \
      (0.5*(SubW-CladW)%WM>0.001) or (0.5*(SubL-CladL)%LM>0.001):                                        
          print(quit)
          quit()
    else:
...
# update the derrordl domain partion
        dp_derrordl_update = dp_derrordl[-1]
...
#                    n_cond += k(0.5*(T+T_conn))*(T_conn-T)/node_distance[n][c_n]*node_cs[n][c_n]
                    n_cond += k(0.5*(T+T_li[node_connected[n][c_n]]),k_T,k_v,index)*(T_li[node_connected[n][c_n]]-T)/node_distance[n][c_n]*node_cs[n][c_n]
#                    try:
#                        n_cond += k(0.5*(T+T_conn))*(T_conn-T)/node_distance[n][c_n]*node_cs[n][c_n]
#                    except:
...
# some interface have different nodal cross section area on oppsite directions; calculate the film condition
            for d in range(3):
                if node_cs[n][2*d] != node_cs[n][2*d+1]:
                    n_film += film(T)*(T-T_air)*(node_cs[n][2*d+1]-node_cs[n][2*d])
...
# laser is only applied at one location with same row and col                     
                ...
                if A_li[node_connected[n][2]]:                    
                    n_cond += k(0.5*(T+T_li[node_connected[n][2]]),k_T,k_v,index)*(T_li[node_connected[n][2]]-T)/node_distance[n][2]*node_cs[n][2]
                else:        
...
#                                 CHM/2.*node_cs[n][2]
                n_cond += k(0.5*(T+T_above),k_T,k_v,index)*(T_above-T)/node_distance[n][2]*node_cs[n_above][2]
            else:
                n_film += film(T)*(T-T_air)*node_cs[n][2]
...
# in abaqus simulation, time increment t_i is 0.05. but this simulation cannot converge with 0.05    
...
#    block_info = block_initialization(geo)
#    nodal_T10, nodal_A10, LSC10, energy_lose10, laser_energy10, median_temperature10 = energy_balance(geo,block_info, t_i, fr, lp, sample_interval)
#    with open('S50C32_FR1000.npy','wb') as f:
#        np.save(f,nodal_T10)
#        np.save(f,nodal_A10)
#        np.save(f,LSC10)
#        np.save(f,energy_lose10)
#        np.save(f,laser_energy10)
#        np.save(f,median_temperature10)
...
# laser power 1.87 kW    
    ...
    geo = geometry(SubH1,SubH2,SubL,SubW,CladH,CladL,CladW,Width,SHM1,SHM2,CHM,LM,WM)
    block_info = block_initialization(geo)
#    nodal_T11, nodal_A11, LSC11, energy_lose11, laser_energy11, median_temperature11 = energy_balance(geo,block_info, t_i, fr, lp, sample_interval)
#    with open('S50C32_FR1100.npy','wb') as f:
#        np.save(f,nodal_T11)
#        np.save(f,nodal_A11)
#        np.save(f,LSC11)
#        np.save(f,energy_lose11)
#        np.save(f,laser_energy11)
#        np.save(f,median_temperature11)
        
# feed rate 1300 mm/min
# laser power 2.21 kW    
#    t_i = 0.01
#    fr  = 1000./60.
#    lp  = 1.
...
#    print('len(nodes_in_layer) = ', len(nodes_in_layer))
#    print('nodes_in_layer[0] = ', nodes_in_layer[0])
     
#    print('node_distance = ', node_distance)
#    print('len(node_distance) = ', len(node_distance))
    
    
    nodal_T, nodal_A, LSC, energy_lose, laser_energy, median_temperature, lp_layer, fr_layer, dp_error, dp_derrordl = \
    energy_balance(geo,block_info, t_i, fr, lp, sample_interval, 'square block',True,True)
#    print('LSC = ', LSC)
    print('median_temperature = ', median_temperature)
    with open('squareblock_test_fr1.7.npy','wb') as f:
        np.save(f,nodal_T)
        np.save(f,nodal_A)
        np.save(f,LSC)
