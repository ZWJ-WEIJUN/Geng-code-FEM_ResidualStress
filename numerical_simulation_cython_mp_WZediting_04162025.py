...
# latent heat
    ...
    for i in range(0, len(cp_T)-1):
        internal_energy.append(
            internal_energy[-1]+(cp_T[i+1]-cp_T[i])*(cp_v[i+1]+cp_v[i])/2.)
...
# initialize node volume
    for n in range(total_nodes+1):
     ...
     # ...........................................................................................................
    with open('thinwall_Geo_CS_lp1.7.npy', 'wb') as f:
        np.save(f, node_dynamic_info)
        np.save(f, node_connected)
        np.save(f, node_distance)
        np.save(f, node_coordinate)
        np.save(f, node_length)
        np.save(f, node_volume)
        np.save(f, node_cs)
        np.save(f, nodes_in_layer)
# ...........................................................................................................
...
# substrate nodes
    nodes_S = int(x_nodes_S*y_nodes_S*z_nodes_S)
# clad nodes
    nodes_C = int((x_nodes_C*z_nodes_C - (x_nodes_C-2*Width/LM-2)
                  * (z_nodes_C-2*Width/WM-2))*y_nodes_C)
# interface nodes
    nodes_I = int(x_nodes_C*z_nodes_C - (x_nodes_C-2*Width/LM-2)
                  * (z_nodes_C-2*Width/WM-2))
# total number of nodes
    total_nodes = nodes_S + nodes_C - nodes_I
...
# *clad*
# number of nodes in a clad layer
    layer_nodes_C = int(x_nodes_C*z_nodes_C - (x_nodes_C -
                        2*Width/LM-2)*(z_nodes_C-2*Width/LM-2))
...
# interface nodes have different upper half volume/cs and lower half volume/cs
    ...
    for n in interface_nodes:
        ...
        for d in horizontal_directions:
            ...
            if conn_above:
                node_cs[n][d] += node_length[n_above][3] * \
                    node_length[n_above][5-d]
    ...
    if (dp_error[layer][0] >= error):
        ...
        while (error > dp_error[layer][i]):
            if (error <= dp_error[layer][i+1]):
                error_p[i] = (dp_error[layer][i+1]-error) / \
                    (dp_error[layer][i+1]-dp_error[layer][i])
    ...
    if (dp_derrordl[layer][0] >= derrordl):
        ...
        while (derrordl > dp_derrordl[layer][i]):
            if (derrordl <= dp_derrordl[layer][i+1]):
                derrordl_p[i] = (dp_derrordl[layer][i+1]-derrordl) / \
                    (dp_derrordl[layer][i+1]-dp_derrordl[layer][i])
...
# c_n: connected node
            for c_n in directions_substrate:
                ...
                if node_connected[n][c_n]:
                    ...
                    n_cond += cmp.k(0.5*(T+T_li[node_connected[n][c_n]]))*(
                        T_li[node_connected[n][c_n]]-T)/node_distance[n][c_n]*node_cs[n][c_n]
#                    try:
#                        n_cond += k(0.5*(T+T_conn))*(T_conn-T)/node_distance[n][c_n]*node_cs[n][c_n]
#                    except:
...
# some interface have different nodal cross section area on oppsite directions; calculate the film condition
            for d in range(3):
                if node_cs[n][2*d] != node_cs[n][2*d+1]:
                    n_film += cmp.film(T)*(T-T_air) * \
                        (node_cs[n][2*d+1]-node_cs[n][2*d])
...
# row and col of the current node
                ...
                if A_li[node_connected[n][2]]:
                    n_cond += cmp.k(0.5*(T+T_li[node_connected[n][2]]))*(
                        T_li[node_connected[n][2]]-T)/node_distance[n][2]*node_cs[n][2]
                else:
...
#                V = node_volume[n]
            if A_li[n_above]:
                ...
                #                                 CHM/2.*node_cs[n][2]
                n_cond += cmp.k(0.5*(T+T_above))*(T_above-T) / \
                    node_distance[n][2]*node_cs[n_above][2]
            else:
                n_film += cmp.film(T)*(T-T_air)*node_cs[n][2]
            ...
            for c_n in directions_clad_top_surface:
                ...
                if node_connected[n][c_n]:
                    ...
                    n_cond += cmp.k(0.5*(T+T_conn))*(T_conn-T) / \
                        node_distance[n][c_n]*node_cs[n][c_n]
...
# skip the loop if the node is inactive and has no laser applied
            if not A_li[n] and not is_laser_on:
                ...
                n_T[n] = T_next(n_E[n], internal_energy,
                                cp_T, cp_v, range_Tnext)
#                q_incr += q
...
#        xc_next, zc_next, layer = clockwise_square(xc,zc,layer,xc0,xc1,zc0,zc1,fr,t_i)
        ...
        if clad_geometry == 'thinwall':
            xc_next, zc_next, layer, ls_dir, dwell_tc = \
                zigzag(xc, zc, layer, ls_dir, dwell_tc, dwell_t, xc0,
                       xc1, zc0, zc1, fr_current, t_i, cladding_direction='x')
        elif clad_geometry == 'square block':
            xc_next, zc_next, layer = clockwise_square(
                xc, zc, layer, xc0, xc1, zc0, zc1, fr, t_i)
...
#                    node_cs[n][d] = node_cs[n][d]*2
            ...
            temp = np.median(
                n_T[(nodes_S+nodes_I*(current_layer-1)):(nodes_S+nodes_I*current_layer)])
...
#            print('derrordl_p = ', derrordl_p)
            ...
            print('median_temperature, layer, current_layer = ',
                  temp, layer, current_layer)
...
#     WM = 1.
    ...
    # geometry size    
    SubH1 = 1.     # substrate top
    SubH2 = 24.    # substrate bottom
    SubL  = 100.    # substrate length
    SubW  = 101    # substrate width
    CladH = 20.    # Thin wall height
    CladL = 80.    # Thin wall length
    CladW = 3.    # cladding width
    Width = 3.     # Thin wall width
    SHM1  = 1.
    SHM2  = 6.
    CHM   = 0.8    # cladding height
    LM    = 1.
    WM    = 1.
...
#                   clad_geometry='thinwall',is_fuzzy_control=True, is_fr_control=True, R=1.541)
    ...
    nodal_T, nodal_A, LSC, energy_lose, laser_energy, median_temperature, lp_layer, fr_layer, dp_error, dp_derrordl = \
        energy_balance(geo, thinwall_info, t_i, fr, lp,
                       sample_interval, 'thinwall', fuzzy_control, True)
    # print('LSC = ', LSC)
