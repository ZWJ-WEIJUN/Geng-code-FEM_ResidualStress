...
ax1.set_ylabel('Laser Power (W)', color=color_lp,size=20)
ax1.tick_params(axis='y', labelcolor='black',labelsize=20)
ax1.set_xlabel("Layer #", fontweight='bold',size=20)
ax1.set_ylabel("Machine Parameters", color='black', fontweight='bold',size=20)
...
plt.title("Combined II Control Step Thin Wall with 1200$^\circ$C Objective", fontname="Arial Black",
...
for index, frame_index_i in enumerate(Frame_index):
    ...
    for j in imge_coord_perLayer:
        ...
        ax.add_patch(square)
        rect = Rectangle((center[0]-15,center[1]-3),31,7, lw=2.0, ec='white', facecolor='none')
        ax.add_patch(rect)
    ...
    # Add a legend
    legend_elements = [
        Patch(facecolor='yellow', edgecolor='none', label='Coordinates from Tranfomration'),
