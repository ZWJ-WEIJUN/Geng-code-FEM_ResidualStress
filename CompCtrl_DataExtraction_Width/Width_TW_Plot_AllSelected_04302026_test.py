...
def plot_graph(dataframes, mid_x_values, colors, markers, alphas, labels):
    ...
    plt.legend(scatterpoints=1, loc='lower center', ncol=1, fontsize=14, markerscale=3)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    fig.savefig('/Users/zhangweijun/Documents/GitHub/Geng-code-FEM_ResidualStress/CompCtrl_DataExtraction_Width/TW_SideProfile_Comparison.png', dpi=300, bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    tw_configs = [
        ('TW2 - No Ctrl', 'WP10_#1_CMM_Cam_B73', 'black', 'o', 1.0),
        ('TW1 - No Ctrl (0.6 mm)', 'WP7_#2_CMM_Cam_B40', 'black', '^', 1.0),
        ...
        ('TW7 - Compr. W-LP Ctrl', 'WP6_#1_CMM_Cam_B40', 'tab:red', 'o', 0.5),
        ('TW8 - Compr. W-LP & FR Hybrid Ctrl', 'WP6_#2_CMM_Cam_B40', 'tab:purple', 'o', 0.45),
        ('TW9 - Compr. W-LP & FR Hybrid Ctrl (Higher Productivity)', 'WP8_#1_CMM_Cam_B40', 'tab:green', 's', 1.0),
    ]
    ...
    print()
    print("=" * 70)
    print("(Height, Width) Data Points for Each Thin Wall")
    print("=" * 70)
    for result in results:
        print(f"\n--- {result['label']} ---")
        print(f"{'Index':>6s} | {'Height (mm)':>12s} | {'Width (mm)':>12s}")
        print("-" * 38)
        for idx, (h, w) in enumerate(zip(result['height_info'], result['width_values'])):
            w_str = f"{w:>12.4f}" if not np.isnan(w) else f"{'NaN':>12s}"
            print(f"{idx:>6d} | {h:>12.4f} | {w_str}")
    print()
    ...
    ax.axhline(y=3.15, color='blue', linestyle='--', linewidth=2.0, label='Target Lower Limit (3.15 mm)')
    ax.axhline(y=3.25, color='blue', linestyle='--', linewidth=2.0, label='Target Upper Limit (3.25 mm)')
    plt.ylim(2.4, 4.5)
    plt.title('Control Method Comparison', fontname='Times New Roman', size=17, fontweight='bold')
    ax.tick_params(axis='both', labelcolor='black', labelsize=17)
    plt.xlabel('Height (mm)', fontweight='bold', size=17)
    ...
    plt.legend(scatterpoints=1, loc='lower center', ncol=1, fontsize=17, markerscale=1.0)
    plt.tight_layout()
    fig3.savefig('/Users/zhangweijun/Documents/GitHub/Geng-code-FEM_ResidualStress/CompCtrl_DataExtraction_Width/TW_Width_Comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
