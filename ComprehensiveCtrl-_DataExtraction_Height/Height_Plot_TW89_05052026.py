...
TW9_HYBRID_HP_CTRL_SHEET_NAME = "WP8_#1_SP_CMM_CMOS_Filtered"
OUTPUT_PATH = Path(
    "/Users/zhangweijun/Documents/GitHub/Geng-code-FEM_ResidualStress/"
    "CompCtrl_DataExtraction_Width/Created_Figures/Height_Profile_TW89.pdf"
)
...
def plot_height_profile(
    ...
    tw9_hybrid_hp_ctrl_df: pd.DataFrame,
) -> None:
    ...
    datasets = [
        ...
        (tw9_hybrid_hp_ctrl_df, "tab:green",  "TW9 - Compr. W-LP & FR Hybrid Ctrl (Higher Productivity)", "o", 1.0),
    ]
...
def main() -> None:
    ...
    tw9_hybrid_hp_ctrl_df = load_height_profile(WORKBOOK_PATH, TW9_HYBRID_HP_CTRL_SHEET_NAME)
    ...
    print(f"Loaded {len(tw9_hybrid_hp_ctrl_df)} valid rows from {TW9_HYBRID_HP_CTRL_SHEET_NAME}")
    print(f"TW9 x range: {tw9_hybrid_hp_ctrl_df['x'].min():.4f} to {tw9_hybrid_hp_ctrl_df['x'].max():.4f}")
    print(f"TW9 y range: {tw9_hybrid_hp_ctrl_df['y'].min():.4f} to {tw9_hybrid_hp_ctrl_df['y'].max():.4f}")
    print(f"Saving figure to: {OUTPUT_PATH}")
    plot_height_profile(tw8_hybrid_ctrl_df, tw9_hybrid_hp_ctrl_df)
