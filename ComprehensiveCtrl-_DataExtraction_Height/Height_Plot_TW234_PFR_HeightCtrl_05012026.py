...
OUTPUT_PATH = Path(
    "/Users/zhangweijun/Documents/GitHub/Geng-code-FEM_ResidualStress/"
    "CompCtrl_DataExtraction_Width/Created_Figures/Height_Profile_TW234.pdf"
)
...
def plot_height_profile(
    tw2_no_ctrl_df: pd.DataFrame,
    tw3_pfr_ctrl_df: pd.DataFrame,
...
def main() -> None:
    ...
    print(f"TW3 x range: {tw3_pfr_ctrl_df['x'].min():.4f} to {tw3_pfr_ctrl_df['x'].max():.4f}")
    print(f"TW3 y range: {tw3_pfr_ctrl_df['y'].min():.4f} to {tw3_pfr_ctrl_df['y'].max():.4f}")
    ...
    plot_height_profile(tw2_no_ctrl_df, tw3_pfr_ctrl_df, tw4_qpf_ctrl_df)
